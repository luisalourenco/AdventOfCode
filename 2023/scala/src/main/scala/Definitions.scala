...
object Definitions:
  ...
  private def formatDay(day: DayNumber): String = f"day_$day%02d"

  private val (inputDirectory, outputDirectory) = ("input", "output")
  
  private def pathForInput(day: DayNumber): String =
    s"${System.getProperty("user.dir")}/../$inputDirectory/${formatDay(day)}.txt"
  
  private def pathForOutput(day: DayNumber, part: PartNumber): String =
    s"${System.getProperty("user.dir")}/../$outputDirectory/${formatDay(day)}-$part.txt"
